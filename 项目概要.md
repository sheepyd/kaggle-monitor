...
* 🎯 **关键词过滤**：只推送你关心的领域（支持正则匹配逻辑），告别无关信息的打扰。
* 📧 **邮件通知**：支持 SMTP 协议（Gmail, Outlook, QQ邮箱, 163邮箱等）。
* 🛡️ **安全配置**：通过 `.env` 环境变量管理敏感信息（API Key, 邮箱密码），保护隐私。
...
```
git clone [https://github.com/YourUsername/kaggle-monitor.git](https://github.com/YourUsername/kaggle-monitor.git)
cd kaggle-monitor
```
...
```
cp .env.example .env
nano .env  # 或者使用 vim / 此处请填入你的配置信息
```

在 `.env` 文件中，你需要重点配置以下三部分（详细说明见下方[配置详解](https://www.google.com/search?q=%23-配置详解)）：

1. **Kaggle API**: 填入你的 Username 和 Key。
2. **Keywords**: 你关注的比赛关键词。
3. **Email**: 你的接收邮箱和 SMTP 发送设置。
...
3. 确认 SMTP 端口（465 通常用于 SSL，587 用于 TLS）。
